...
class PlayEntry:
    """Contains the time, team, team score, opponent score and possession."""
...
def seconds_elapsed(entry, headers):
    ...
    assert 0 < period <= 8, 'period {0} does not appear valid'.format(period)
...
def extract_nba_pbp_entries():
    ...
    games_skipped = 0
    games = []
    cache_file = 'data/cache/nba_pbp_game_cache.p'
    ...
    for game_file in glob.glob('data/nba_stats/*pbp*.json'):
        ...
        try:
            ...
            logging.exception('skipping {0} because {1}'.format(game_file, e))
            games_skipped += 1
        # if games_examined > 100:
        #     break
    if games_skipped > 0:
        logging.warning('Skipped {0} of {1} games because of errors'.format(games_skipped, games_examined))
    pickle.dump(games, open(cache_file, 'wb'))
    return games

if __name__ == '__main__':
    extract_nba_pbp_entries()
